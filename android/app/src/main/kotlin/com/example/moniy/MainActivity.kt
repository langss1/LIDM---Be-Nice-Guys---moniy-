...
class MainActivity: FlutterActivity() {
    private val CHANNEL = "com.example.moniy/accessibility"
        ...
        methodChannel?.setMethodCallHandler { call, result ->
            when (call.method) {
                "isAccessibilityEnabled" -> {
                    val expectedComponentName = ComponentName(context, MoniyAccessibilityService::class.java)
                ...
                "triggerNativeBlock" -> {
                    val blocked = MoniyAccessibilityService.instance?.triggerBlocking()
                    if (blocked == true) {
                        result.success(true)
                    } else {
                ...
                "performGlobalActionBack" -> {
                    val intent = Intent("com.example.moniy.PERFORM_BACK")
                    context.sendBroadcast(intent)
                    result.success(true)
                }
                "setNativeProtection" -> {
                    val isActive = call.arguments as Boolean
                    MoniyAccessibilityService.isProtectionActive = isActive
                    result.success(true)
                }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = IntentFilter("com.example.moniy.SEND_TEXT")
